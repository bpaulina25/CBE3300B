...
lv_obj_t * loadingBar;
lv_obj_t * infoLabel;
lv_obj_t * ta_new_user;
lv_obj_t * kb_new_user;
const char * currentUser = "Pau";

void loadMainUIScreen();
void loadIntroScreen();
void resetToIntro();
void refreshConcentration();
void showKeyboardForNewUser();
...
void kb_event_cb(lv_event_t * e) {
  ...
  if (code == LV_EVENT_VALUE_CHANGED) {
    ...
    currentUser = lv_textarea_get_text(ta);
    lv_obj_del(kb_new_user);
    lv_obj_del(ta_new_user);
  }
}

void showKeyboardForNewUser() {
  ta_new_user = lv_textarea_create(lv_scr_act());
  lv_obj_set_width(ta_new_user, 200);
  lv_obj_align(ta_new_user, LV_ALIGN_TOP_MID, 0, 20);
  lv_textarea_set_placeholder_text(ta_new_user, "Enter name");
  lv_textarea_set_one_line(ta_new_user, true);

  kb_new_user = lv_keyboard_create(lv_scr_act());
  lv_keyboard_set_mode(kb_new_user, LV_KEYBOARD_MODE_TEXT_LOWER);
  lv_keyboard_set_textarea(kb_new_user, ta_new_user);
  lv_obj_add_event_cb(kb_new_user, kb_event_cb, LV_EVENT_ALL, ta_new_user);
}
...
void loadMainUIScreen() {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    if (i == 1) lv_obj_add_state(cb, LV_STATE_CHECKED);
    if (i == 5) lv_obj_add_event_cb(cb, [](lv_event_t * e){
      showKeyboardForNewUser();
